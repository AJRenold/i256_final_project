...
logging.basicConfig(filename='log_file18112013.log',level=logging.WARNING, format='%(asctime)s %(message)s')

def main():
    reporter = Reporter()
    logging.warning('Crawler started . . .')
    ...
    while count > 0:
        ...
        with con:
            ...
            results = [ r for r in islice(cur.fetchall(),100) ]
            logging.warning('fetched next ' + str(len(results)))
            con.commit()
        ...
        with con:
            ...
            logging.warning('on crawler step')
            for result in results:
                last = result['id']
                print 'crawling ', result['link']
                try:
                    url_visited, page_text = fetchText(result['link'], reporter)
                except Exception as e:
